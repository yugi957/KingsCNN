...
def detect_corners(image):
    ...
    model_trained = YOLO("best_ashwood_weights.pt")
    results = model_trained.predict(source=image, line_thickness=1, conf=0.25)
    # Show the results
    im = []
    for r in results:
...
def chess_pieces_detector(image):
    ...
    results = model_trained.predict(source=image, line_thickness=1, conf=0.25, augment=False)
    # Show the results
    im = []
    for r in results:
        im_array = r.plot()  # plot a BGR numpy array of predictions
...
def transform_image(image):
    transform = transforms.Compose([
        ...
        transforms.Resize((640, 640)),  # Resize the image to the required input size of the model.
        # Add any other transforms you need for your model.
    ])
    return transform(image)

def to_FEN(img, debug=False):
    # image = 'images/test_4.jpg'
    # image = transform_image(img)
    # image = image.unsqueeze(0)  # Add a batch dimension
    image = img

    corners, corner_image = detect_corners(image)

    transformed_image = four_point_transform(image, corners)

    ptsT, ptsL = plot_grid_on_transformed_image(transformed_image)

    detections, boxes, bounded_image = chess_pieces_detector(transformed_image)
    # print("boxes:")
    # print(boxes)
    if(debug == 'corners'): return corner_image
    ...
    if(debug == 'pieces'): return bounded_image
